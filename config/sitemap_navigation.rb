SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |primary|
    MainPage.all.each do |main_page|
      primary.item :"main_page_#{main_page.id}", main_page.title, present(main_page).path do |first_level|
        all_parent_pages[main_page.locale.to_sym].each do |parent_page|
          first_level.item :"parent_page_#{parent_page.id}",
                            parent_page.title,
                            present(parent_page).path do |second_level|
            all_child_pages[parent_page.id].each do |child_page|
              second_level.item :"child_page_#{child_page.id}", child_page.title, present(child_page).path
            end
          end
        end
      end
